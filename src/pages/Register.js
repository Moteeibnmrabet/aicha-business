...
function RegisterForm({ onShowToast }) {
      ...
      <div className="max-w-md w-full">
        <div className="bg-white p-8 border border-light-gray">
          ...
          <p className="text-center text-deep-black/60 font-sans text-sm mb-8">
            Inscrivez-vous pour effectuer vos achats
          </p>
          ...
          <p className="mt-8 text-center font-sans font-light text-deep-black/70 text-sm">
            Déjà un compte ?{' '}
            <Link to="/login" state={{ from: location.state?.from }} className="underline hover:text-deep-black">
              Se connecter
            </Link>
          </p>
        </div>
  ...
  if (config.googleClientId) {
    return (
      <GoogleOAuthProvider clientId={config.googleClientId}>
        <RegisterForm onShowToast={onShowToast} />
      </GoogleOAuthProvider>
    );
  }
  return <RegisterForm onShowToast={onShowToast} />;
}
